...
public class Swerve extends SwerveDriveSubsystem {
    ...
    public void onUpdate() {
        if (!limeLight.getEstimate().isPresent() || limeLight.getEstimate().get().tagCount <= 0 || _pigeon.getAngularVelocity() >= 720) {
            poseRejected = true;
        }
        ...
        else if(Math.hypot(getRobotRelativeSpeeds().vxMetersPerSecond, getRobotRelativeSpeeds().vyMetersPerSecond) > 3){
            poseRejected = true;
        }
        else if(Math.abs(Math.hypot(getPose().getTranslation().getX(), getPose().getTranslation().getY()) - Math.hypot(limeLight.getEstimate().get().pose.getX(), limeLight.getEstimate().get().pose.getY())) > kMaxVisionCorrection){
            poseRejected = true;
        }
        else {
            poseRejected = false;
        ...
        if(!poseRejected){
            xyStdDev = xyStdDevCoeff * Math.pow(limeLight.getEstimate().get().distToTag, 2.0) / limeLight.getEstimate().get().tagCount;
            rStdDev = rStdDevCoeff * Math.pow(limeLight.getEstimate().get().distToTag, 2.0) / limeLight.getEstimate().get().tagCount;
        ...
        if (!limeLight2.getEstimate().isPresent() || limeLight2.getEstimate().get().tagCount <= 0 || _pigeon.getAngularVelocity() >= 720) {
            poseRejected2 = true;
        }
        else if (limeLight2.getEstimate().get().pose.getX() < -kFieldBorderMargin || limeLight2.getEstimate().get().pose.getX() > 52.08333 + kFieldBorderMargin || limeLight2.getEstimate().get().pose.getY() < -kFieldBorderMargin || limeLight2.getEstimate().get().pose.getY() > 26.583333333 + kFieldBorderMargin){
            poseRejected2 = true;
        }
        else if(Math.hypot(getRobotRelativeSpeeds().vxMetersPerSecond, getRobotRelativeSpeeds().vyMetersPerSecond) > 3){
            poseRejected2 = true;
        }
        else if(Math.abs(Math.hypot(getPose().getTranslation().getX(), getPose().getTranslation().getY()) - Math.hypot(limeLight2.getEstimate().get().pose.getX(), limeLight2.getEstimate().get().pose.getY())) > kMaxVisionCorrection){
            poseRejected2 = true;
        }
        else {
            poseRejected2 = false;
        ...
        if(!poseRejected2){
            xyStdDev = xyStdDevCoeff * Math.pow(limeLight2.getEstimate().get().distToTag, 2.0) / limeLight2.getEstimate().get().tagCount;
            rStdDev = rStdDevCoeff * Math.pow(limeLight2.getEstimate().get().distToTag, 2.0) / limeLight2.getEstimate().get().tagCount;
            super.addVisionMeasurement(limeLight2.getEstimate().get().pose, limeLight2.getEstimate().get().time,
                    VecBuilder.fill(xyStdDev, xyStdDev, rStdDev));
        }

        limeLight.SetRobotOrientation(getPose().getRotation(), _pigeon.getAngularVelocity());
